...
public class RegisterPlayerImpl implements Command {
	...
	public String execute(String request, GameCrossesZeros theGame) {
		...
		msg.append("Hi, ");
		if (theGame.getResultGame() == -2) { 
			theGame.setPlayer1Name(params[1]);
			msg.append ( theGame.getPlayer1Name()+". Please, wait for another player." );
		} else if (theGame.getResultGame() == -1) {
			theGame.setPlayer2Name(params[1]);
			msg.append( theGame.getPlayer2Name()+"." + ( theGame.getWhatPlayerMovement()==2 ? "Make you move, please." : "Please, wait for another player moving." ) );
		}	
     	return msg.toString();
	}	
